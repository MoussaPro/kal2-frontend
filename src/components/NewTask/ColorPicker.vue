<template>
  <div class="relative">
    ...
    <div v-if="toggleColors" class="absolute mt-4 gap-x-2 flex ml-5 bg-gray-50 p-3 shadow-xl p-3 rounded hover-transition z-20" v-click-outside="hideColor">
      ...
      <div v-for="color in colors" :class="color.css" class="w-10 h-10 rounded shadow-md hover-transition cursor-pointer hover:scale-[103%] hover:shadow-xl" v-show="activeColor.name !== color.name" @click="setActiveColor(color); $emit('color', activeColor.name)"></div>
    </div>
  </div>
</template>
<script setup>
